...
def create_tool(func: Callable) -> Tool:
    ...
    def wrapped_func(**kwarg):
        ...
        print(f"DEBUG mcp_utils: Received headers = {dict(original_headers)}", file=sys.stderr, flush=True)
        ...
        print(f"DEBUG mcp_utils: Extracted - id_token={'SET' if id_token else 'NONE'}, auth_vo={auth_vo}, oidc={oidc}", file=sys.stderr, flush=True)

        # set the id_token and auth_vo in the HttpClient
        http_client = HttpClient()
        http_client.override_oidc(oidc, id_token, auth_vo)
        
        print(f"DEBUG mcp_utils: Calling {http_method.upper()} {url}", file=sys.stderr, flush=True)
        status, output = getattr(http_client, http_method)(url, kwarg)
        print(f"DEBUG mcp_utils: Response status={status}", file=sys.stderr, flush=True)
